...
import { receiveTOLines } from "@/server/actions/transfer-orders";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { useRouter } from "next/navigation";

interface Line { id: string; sku: string; name: string; shipped: number; received: number }

export function TOReceiveForm({ toId, lines }: { toId: string; lines: Line[] }) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [quantities, setQuantities] = useState<Record<string, number>>(
    Object.fromEntries(lines.map((l) => [l.id, Math.max(0, l.shipped - l.received)]))
  );
  const [error, setError] = useState("");

  function handleReceive() {
    setError("");
    const receipts = lines.map((l) => ({ lineId: l.id, quantityReceiving: quantities[l.id] ?? 0 })).filter((r) => r.quantityReceiving > 0);
    if (receipts.length === 0) { setError("Enter at least one quantity to receive."); return; }
    startTransition(async () => {
      try { await receiveTOLines(toId, receipts); router.refresh(); }
    ...
    <div className="space-y-3">
      <table className="w-full text-sm">
        <thead><tr className="text-xs text-slate-500 border-b border-slate-200">
          ...
          <th className="text-right pb-2 font-medium">In Transit</th>
          <th className="text-right pb-2 font-medium w-28">Receiving Now</th>
        </tr></thead>
        <tbody>
          {lines.map((line) => {
            const remaining = line.shipped - line.received;
            ...
            return (
              <tr key={line.id} className="border-b border-slate-50">
                ...
                <td className="py-2 text-right">
                  <Input type="number" min={0} max={remaining} value={quantities[line.id] ?? 0}
